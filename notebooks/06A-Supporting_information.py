...
# # Stratified groups for "All training"

# In[193]:


groups = ['Other', 'Acylation', 'Concatenated proteins',
       'Antibody attachment (Fc)', 'AA extensions']
combine_within_group = pd.DataFrame()
nncds_test_data = {}
...
for group in groups:
    ...
    for fold in folds:
        ...
        tmp = pd.DataFrame(Output_scatter_dict[group][Output_scatter_dict[group]["Fold"] == fold]["nncno"].unique(),columns = ["nncno"])
        tmp["Fold"] = fold
        combine_within_group = pd.concat([combine_within_group,tmp],axis=0)
        nncds_test_data[group] = combine_within_group
...
forced_nncds_in_test = nncds_test_data["Other"][nncds_test_data["Other"]["Fold"] == 0]["nncno"]
nncd_sample_test_pool = data_wg[data_wg["Groups"] != "Other"]

sampled_nncds_from_test = np.random.choice(nncd_sample_test_pool.index,128-forced_nncds_in_test.shape[0])

nncd_group_test_forced = data_wg[data_wg.index.isin(forced_nncds_in_test)]
nncd_group_test_sampled = data_wg[data_wg.index.isin(sampled_nncds_from_test)]
Test_set = pd.concat([nncd_group_test_forced,nncd_group_test_sampled],axis=0)
Train_val_set = data_wg[~data_wg.index.isin(Test_set.index)]

hej = {"Other": [Train_val_set,Test_set,0]}


# In[171]:


forced_nncds_in_test = nncds_test_data['AA extensions'][nncds_test_data[group]["Fold"] == fold]["nncno"]
nncd_sample_test_pool = data_wg[data_wg["Groups"] != 'AA extensions']
...
for group in groups:
    ...
    for fold in folds:
        ...
        forced_nncds_in_test = nncds_test_data[group][nncds_test_data[group]["Fold"] == fold]["nncno"]
        nncd_sample_test_pool = data_wg[data_wg["Groups"] != group]
        sampled_nncds_from_test = np.random.choice(nncd_sample_test_pool.index,128-forced_nncds_in_test.shape[0])
        ...
        Output[fold] = [Train_val_set,Test_set,fold]
        
    total_output_dict[group] = Output
...
Full_data.set_index("nncno",inplace=True)
PK_names = ["CL[ml/min/kg]","T1/2[h]","MRT[h]"]
PK_names_group = ["CL[ml/min/kg]","T1/2[h]","MRT[h]","Groups"]
...
Output_scatter_dict = {}
# Extract data
data = contruct_descriptor_sets(Full_data,pd.Series("124"),with_PK = True)
# Split into response and descriptors
X_vivo = data.drop(PK_names,axis=1)
Y_vivo = data[PK_names]
Y_vivo = np.log(Y_vivo)
...
for group in groups:
    ...
    for j in range(0,5):
        ...
        X_train_outer = total_output_dict[group][j][0].drop(PK_names_group,axis=1)
        Y_train_outer = total_output_dict[group][j][0][PK_names_group]
        X_test = total_output_dict[group][j][1].drop(PK_names_group,axis=1)
        Y_test = total_output_dict[group][j][0][PK_names_group]
...
# Save ouput files
with open('../data/processed/Random_forest_outer_test_scorer_stratified.pkl','wb') as f:pickle.dump(Output_dict,f )
with open('../data/processed/Random_forest_scatter_file_stratified.pkl','wb') as f:pickle.dump(Output_scatter_dict,f )
with open('../data/processed/Random_forest_hyperparameter_file_stratified.pkl','wb') as f:pickle.dump(Output_Hyperparameter_dict,f )
...
RF_DS124_scatter= pickle.load(open('../data/processed/Random_forest_scatter_file_stratified.pkl','rb'))
...
all_groups_RF124 = pd.DataFrame()
for group in groups:
    tmp = RF_DS124_scatter[group]
    all_groups_RF124 = pd.concat([all_groups_RF124,tmp],axis=0)
       
global_model_DS124 = all_groups_RF124
...
groups = ['Other', 'Acylation', 'Concatenated proteins', 'Antibody attachment (Fc)', 'AA extensions']
for i in groups:
    print("====  {} =====".format(i))
